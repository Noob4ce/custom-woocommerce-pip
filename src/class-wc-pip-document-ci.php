...
class WC_PIP_Document_CI extends WC_PIP_Document {
...
public function document_header( $type, $action, $document, $order ) {
    ...
    if ( ! $order_id || 'ci' !== $type || ( ( (int) $order_id !== (int) $this->order_id ) && has_action( 'wc_pip_header', array( $this, 'document_header' ) ) ) ) {
        return;
    }
